package com.hz.zxk.superview_kotlin.textView
...
class SuperTextView @JvmOverloads constructor(
    ...
    init {
        if (attrs != null) {
            val ta = context.obtainStyledAttributes(attrs, R.styleable.SuperTextView)
            try {
                //按钮弧度
                radius = ta.getDimensionPixelOffset(R.styleable.SuperTextView_st_radius, radius)
                if (radius == 0) {
                    topLeftRadius = ta.getDimensionPixelOffset(
                        R.styleable.SuperTextView_st_topLeftRadius,
                        topLeftRadius
                    )
                    topRightRadius = ta.getDimensionPixelOffset(
                        R.styleable.SuperTextView_st_topRightRadius,
                        topRightRadius
                    )
                    bottomLeftRadius = ta.getDimensionPixelOffset(
                        R.styleable.SuperTextView_st_bottomLeftRadius,
                        bottomLeftRadius
                    )
                    bottomRightRadius = ta.getDimensionPixelOffset(
                        R.styleable.SuperTextView_st_bottomRightRadius,
                        bottomRightRadius
                    )
                } else {
                ...
                //按钮背景
                normalBackground =
                    ta.getColor(R.styleable.SuperTextView_st_normalBackground, normalBackground)
                pressedBackground =
                    ta.getColor(R.styleable.SuperTextView_st_pressedBackground, pressedBackground)
                selectedBackground =
                    ta.getColor(R.styleable.SuperTextView_st_selectedBackground, selectedBackground)
                enabledBackground =
                    ta.getColor(R.styleable.SuperTextView_st_enabledBackground, enabledBackground)
                //按钮背景样式
                backgroundStyle =
                    ta.getInteger(R.styleable.SuperTextView_st_backgroundStyle, backgroundStyle)
                normalBackgroundStyle = ta.getInteger(
                    R.styleable.SuperTextView_st_normalBackgroundStyle,
                    normalBackgroundStyle
                )
                pressedBackgroundStyle = ta.getInteger(
                    R.styleable.SuperTextView_st_pressedBackgroundStyle,
                    pressedBackgroundStyle
                )
                selectedBackgroundStyle = ta.getInteger(
                    R.styleable.SuperTextView_st_selectedBackgroundStyle,
                    selectedBackgroundStyle
                )
                enabledBackgroundStyle = ta.getInteger(
                    R.styleable.SuperTextView_st_enabledBackgroundStyle,
                    enabledBackgroundStyle
                )
                //边框宽度
                storkWidth =
                    ta.getDimensionPixelOffset(R.styleable.SuperTextView_st_storkWidth, storkWidth)
                normalStorkWidth = ta.getDimensionPixelOffset(
                    R.styleable.SuperTextView_st_normalStorkWidth,
                    storkWidth
                )
                pressedStorkWidth = ta.getDimensionPixelOffset(
                    R.styleable.SuperTextView_st_pressedStorkWidth,
                    storkWidth
                )
                selectedStorkWidth = ta.getDimensionPixelOffset(
                    R.styleable.SuperTextView_st_selectStorkWidth,
                    storkWidth
                )
                enabledStorkWidth = ta.getDimensionPixelOffset(
                    R.styleable.SuperTextView_st_enabledStorkWidth,
                    storkWidth
                )
                //按钮字体颜色
                normalTextColor =
                    ta.getColor(R.styleable.SuperTextView_st_normalTextColor, normalTextColor)
                pressedTextColor =
                    ta.getColor(R.styleable.SuperTextView_st_pressedTextColor, pressedTextColor)
                selectedTextColor =
                    ta.getColor(R.styleable.SuperTextView_st_selectedTextColor, selectedTextColor)
                enabledTextColor =
                    ta.getColor(R.styleable.SuperTextView_st_enabledTextColor, enabledTextColor)
            } finally {
                ta.recycle()
            }
        }
